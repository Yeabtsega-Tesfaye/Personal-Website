...
function initMagneticButtons() {
  ...
  if (window.innerWidth <= 768) {
    ...
    document
      .querySelectorAll(".btn-magnetic, .magnetic-container")
      .forEach((el) => {
  ...
  const getButtonConfig = (button) => {
    const baseConfig = {
      ...
      particleColor: button.classList.contains("btn-primary")
        ? "accent"
        : "white",
    };
  ...
  class MagneticButton {
    ...
    updatePosition() {
      ...
      this.currentX +=
        (this.targetX - this.currentX) * this.config.movementDamping;
      this.currentY +=
        (this.targetY - this.currentY) * this.config.movementDamping;
      ...
      const normalizedDistance = Math.min(
        distance / this.config.magneticRadius,
        1,
      );
      ...
      const clampedX = Math.max(
        Math.min(finalX, this.config.maxMovement),
        -this.config.maxMovement,
      );
      const clampedY = Math.max(
        Math.min(finalY, this.config.maxMovement),
        -this.config.maxMovement,
      );
      ...
      const shouldBeActive =
        distance < this.config.magneticRadius && magneticForce > 0.2;
    ...
    handleMouseMove(e) {
      const {
        ...
        y: deltaY,
        distance,
      } = this.getDistanceFromCenter(e.clientX, e.clientY);
...
class CompleteSoundSystem {
  ...
  setupTerminalTypingSounds() {
    ...
    console.log(
      "⌨️ Terminal sounds: Click anywhere in terminal for keyboard sound",
    );
  }
...
function initTerminal() {
  ...
  const terminalScript = [
    ...
    },
    {
      command: "ls skills/",
      output:
        "<span class='dir'>backend/</span>  <span class='dir'>frontend/</span>  <span class='dir'>databases/</span>  <span class='file'>tools.js</span>",
      delay: 800,
    },
    {
      ...
      typeText(commandSpan, command, terminalConfig.typingSpeed, () => {
        ...
        setTimeout(() => {
          if (output) {
            ...
            // Scroll to bottom
            terminalContent.parentElement.scrollTop =
              terminalContent.parentElement.scrollHeight;
          }
  ...
  async function runTerminalScript() {
    ...
    const initialLines = document.querySelectorAll(
      ".terminal-line:not(.active-line), .terminal-output",
    );
    ...
    for (let i = 0; i < terminalScript.length; i++) {
      ...
      if (!isLast) {
        await new Promise((resolve) =>
          setTimeout(resolve, terminalConfig.lineDelay),
        );
      }
    }
...
Type <span class="accent">help</span> for available commands.
  ...
  function initScrollReveal() {
    document
      .querySelectorAll(".portfolio-card, .blog-card, .service-card")
      .forEach((card, index) => {
  ...
  function initCursorEffect() {
    ...
    const interactiveElements = document.querySelectorAll(
      "a, button, .btn, .portfolio-card, .service-card",
    );
...
window.addEventListener("load", function () {
  const loader = document.querySelector(".silent-loader");
  document.documentElement.classList.add("loader-open");

  // Wait for animation to finish (about 1 second)
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      // REMOVE THE CLASS HERE - after the loader is completely hidden
      document.documentElement.classList.remove("loader-open");
    }, 500);
  }, 1000); // 1 second delay to see the animation
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initialize terminal afer waiting the loader animation
  setTimeout(() => {
    initTerminal();
  }, 500);
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const allSkills = [
    // Languages
    {
      ...
      category: "Language",
      description:
        "The language I live in; turning ideas into interactive apps.",
      experience: "3+ years",
      level: "Daily use",
  ...
  function openSkillPopup(skillName) {
    const skill =
      allSkills.find((s) => s.name.toLowerCase() === skillName) || allSkills[0];
    const popup = document.getElementById("skillPopup");
    ...
    if (skill.icon.includes(".svg")) {
      document.getElementById("popupIcon").innerHTML =
        `<img src="${skill.icon}" alt="${skill.name}" class="skill-svg-icon-pop">`;
    } else {
      document.getElementById("popupIcon").innerHTML =
        `<i class="${skill.icon}"></i>`;
    }
